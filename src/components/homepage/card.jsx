...
export default function Card({name, desc, lang, pic, pageLink}) {
    ...
    return (
        <div className="flex flex-col justify-center items-center mt-20" onClick={handleClick}>     
        
            <h1 className='text-xl text-primaryBlue font-cyber'> {name} </h1>
           {/*  <small className='text-md text-secondaryPurple font-main'> {lang} </small> */}
            
            {
               ...
               onMouseLeave={() => setHover(false)}
                ...
                <img className={hover ? "mt-5 w-full bg-red-50 rounded-lg md:h-4/5 xl:h-5/6 " : "mt-5 h-2/3 w-full bg-red-50 rounded-lg md:h-4/5 xl:h-5/6"}  
                height={210}
                width={350}
                src={pic} 
                onMouseEnter={() => setHover(true)}
